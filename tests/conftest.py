...
def sample_inpatient(sample_beneficiaries) -> pd.DataFrame:
    ...
    admit_dates = pd.to_datetime(
        pd.date_range("2008-01-01", "2010-12-31", periods=n_claims)
    )
    ...
    return pd.DataFrame({
        ...
        "ICD9_DGNS_CD_2": [f"{rng.integers(100,999)}" if rng.random() > 0.3 else None for _ in range(n_claims)],
    })
...
def sample_conditions(sample_beneficiaries) -> pd.DataFrame:
    ...
    CONDITION_MAP = {
        ...
        "SP_OSTEOPRS": "osteoporosis", "SP_RA_OA": "ra_oa", "SP_STRKETIA": "stroke_tia",
    }
    rng = np.random.default_rng(42)
    rows = []
