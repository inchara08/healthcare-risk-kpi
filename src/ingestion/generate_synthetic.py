    ...
    df = pd.DataFrame(
        {
            ...
            "BENE_RACE_CD": rng.choice(
                [1, 2, 3, 4, 5, 6], size=n, p=[0.82, 0.10, 0.02, 0.02, 0.03, 0.01]
            ).astype(int),
...
def generate_inpatient_claims(
    ...
    for patient_idx in range(len(bene_ids)):
        ...
        for admit_dt in admit_dts:
            ...
            prior_90d = sum(1 for prev in recorded_admit_dts if 0 < (admit_dt - prev).days <= 90)
            ...
            if risk_factor > 0.6:
                disch_status = rng.choice(
                    [1, 2, 3, 6, 20, 30], p=[0.27, 0.14, 0.24, 0.16, 0.11, 0.08]
                )
            elif risk_factor > 0.3:
                disch_status = rng.choice(
                    [1, 2, 3, 6, 20, 30], p=[0.47, 0.12, 0.15, 0.12, 0.08, 0.06]
                )
            else:
                disch_status = rng.choice(
                    [1, 2, 3, 6, 20, 30], p=[0.70, 0.08, 0.07, 0.08, 0.04, 0.03]
                )
            ...
            row: dict = {
                ...
                "NCH_BENE_BLOOD_PNTS_FRNSHD_QTY": int(
                    rng.choice([0, 1, 2, 3], p=[0.85, 0.08, 0.04, 0.03])
                ),
            ...
            if rng.random() < readmit_prob and discharge_dt < pd.Timestamp("2010-12-01"):
                ...
                readmit_row["NCH_BENE_DSCHRG_DT"] = readmit_admit + pd.Timedelta(days=readmit_los)
...
def generate_and_save(
    ...
    logger.info("Saved beneficiaries: %s (%d rows)", bene_path.name, len(bene_df))
    logger.info(
